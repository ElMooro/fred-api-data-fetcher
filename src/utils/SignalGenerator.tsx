...
const calculateRSI = (data: DataPoint[], periods = 14) => {
...
const calculateEMA = (values: (number | null)[], period: number) => {
  const k = 2 / (period + 1);
...
const calculateBollingerBands = (data: DataPoint[], period = 20, stdDev = 2) => {
...
export const generateSignals = (data: DataPoint[], selectedMetrics: string[]) => {
    ...
    if (index >= 50) {  // Ensure we have enough data for SMA50
      ...
      selectedMetrics.forEach(metric => {
        ...
        if (metric === "MACD" && macd.histogram[index] !== null) {
          totalMetrics++;
          if ((macd.histogram[index] as number) > 0 && (macd.histogram[index - 1] as number) <= 0) {
            buySignals++;
            signals.push({ metric: "MACD", signal: "buy", value: macd.histogram[index] as number });
          } else if ((macd.histogram[index] as number) < 0 && (macd.histogram[index - 1] as number) >= 0) {
            sellSignals++;
            signals.push({ metric: "MACD", signal: "sell", value: macd.histogram[index] as number });
          }
        }
        
        if (metric === "SMA" && sma50[index] !== null && sma200[index] !== null) {
          totalMetrics++;
          if ((sma50[index] as number) > (sma200[index] as number) && (sma50[index - 1] as number) <= (sma200[index - 1] as number)) {
            buySignals++;
            signals.push({ metric: "SMA", signal: "buy", value: `${(sma50[index] as number).toFixed(2)}/${(sma200[index] as number).toFixed(2)}` });
          } else if ((sma50[index] as number) < (sma200[index] as number) && (sma50[index - 1] as number) >= (sma200[index - 1] as number)) {
            sellSignals++;
            signals.push({ metric: "SMA", signal: "sell", value: `${(sma50[index] as number).toFixed(2)}/${(sma200[index] as number).toFixed(2)}` });
          }
        }
        
        if (metric === "BB" && bollingerBands[index].bPercent !== null) {
          totalMetrics++;
          if ((bollingerBands[index].bPercent as number) <= 0.2) {
            buySignals++;
            signals.push({ metric: "Bollinger Bands", signal: "buy", value: (bollingerBands[index].bPercent as number).toFixed(2) });
          } else if ((bollingerBands[index].bPercent as number) >= 0.8) {
            sellSignals++;
            signals.push({ metric: "Bollinger Bands", signal: "sell", value: (bollingerBands[index].bPercent as number).toFixed(2) });
          }
        }
      });
    }
...
export const identifyCrisisPoints = (data: DataPoint[]) => {
